        ...
        <div class="easyui-panel" id="DataList" title="活动信息"  data-options="iconCls:'fi-results'">
            <c:forEach items="${oRefundPriceDiff.refundPriceDiffDetailList}" var="refundPriceDiffDetail">
                <table class="grid editGrid">
                    ...
                    <tr>
                        <td>商品名称：</td>
                        <td>${refundPriceDiffDetail.proName}</td>
                        <td>价格：</td>
                        <td>${refundPriceDiffDetail.price}</td>
                        <td>数量：</td>
                        <td class="changeCountClass">${refundPriceDiffDetail.changeCount}</td>
                        <td>活动名称：</td>
                        <td class="cActName" proId="${refundPriceDiffDetail.proId}" agentId="${refundPriceDiffDetail.agentId}">
                        </td>
                    </tr>
                    <tr>
                        <td>订单id：</td>
        ...
        $("select[name='activityRealId']").change(function(){
            $.ajaxL({
                ...
                success: function(msg){
                    $("#bcAmt").html("<td>补差金额:</td><td>"+msg+"元</td>");
                },
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $.each(data,function(n,item) {
                        orderStr+="<option value='"+item.id+"' >"+item.activityName+"</option>";
                    });
                    orderStr+="</select>";
