...
<div>
    ...
    <div  class="easyui-panel" title="订单信息" style="width: 100%;background:#fafafa;" data-options="">
        <table  class="grid">
            <tbody>
                <tr>
                    <td style="width: 50px;text-align: right;">代理商:</td><td>${data.agent.agName}</td>
                    <td style="width: 50px;text-align: right;">编号:</td><td>${data.order.oNum}</td>
                    <td style="width: 50px;text-align: right;">金额:</td><td>${data.order.oAmo}/元</td>
                </tr>
                <tr>
                    <td style="width: 50px;text-align: right;">差价:</td><td>${data.order.incentiveAmo}/元</td>
                    <td style="width: 50px;text-align: right;">应付:</td><td>${data.order.payAmo}/元</td>
                    <td style="width: 50px;text-align: right;">日期:</td><td><fmt:formatDate pattern="yyyy-MM-dd" value="${data.order.cTime}" /></td>
                </tr>
                <tr>
                    <td style="width: 50px;text-align: right;">状态:</td><td><c:forEach items="${orderStatus}" var="orderStatusItem"> <c:if test="${data.order.orderStatus == orderStatusItem.dItemvalue}">${orderStatusItem.dItemname}</c:if></c:forEach></td>
                    <td style="width: 50px;text-align: right;">备注:</td><td colspan="3">${data.order.remark}</td>
                </tr>
            </tbody>
        </table>
    </div>
        ...
        <table  class="grid">
            <tbody>
                ...
                <tr>
                    ...
                    <td>${data.oPayment.settlementPrice}</td>
                ...
                <tr>
                    ...
                    <td>
                        <c:if test="${oPaymentDetailsItem.paymentStatus==1}">
                            <a href="javascript:;" onclick="bukuanAction_orderview('srcId=${oPaymentDetailsItem.id}&pkType=1&remark=OrderNum-${data.order.oNum}&agentId=${data.order.agentId}&payAmount=${oPaymentDetailsItem.payAmount}')">补款</a>
                        </c:if>
                    </td>
                </tr>
                </c:forEach>
